...
- run context（session/run/item）存在进程全局（MCP 工具调用落在不同线程，
  thread-local 无法跨调用传递），通过 minium_tools.set_context_provider 注入；
...
_server_ctx = {"session_id": "", "run_id": "", "item_id": ""}


def install_context_provider() -> None:
    """把进程全局上下文接进 minium_tools 的 provider 注入点（main 启动时调用；
    测试中显式调用/卸载，避免污染同进程的 in-process 测试）。"""
    minium_tools.set_context_provider(lambda: dict(_server_ctx))


def set_run_context(session_id: str, run_id: str, item_id: str) -> str:
    """设置当前检查执行的上下文（session/run/item）：screenshot 据此拼存档路径，
    run_id 变化时 minium 会话按 run 隔离重建。由后端 executor 每项开始前代码直调
    （不经 LLM），后端不可见工具 id 细节。"""
    with _ctx_lock:
        _server_ctx.update(session_id=session_id, run_id=run_id, item_id=item_id)
    return "ok"
