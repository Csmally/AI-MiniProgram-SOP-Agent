...
> 创建时间：2026-08-12 | 最后更新：2026-08-17
...
| AI 能力 | DeepSeek API（结构化输出 + 流式对话）+ Qwen（截图视觉分析） |
...
主图 START ──router──▶ 所有操作唯一入口（按 next_action 分发）
       ...
       ├─ approve/run → dispatch → execute_agent(子图) 自循环 ×N（游标串行执行）
       │               → collect 汇总 → report_agent(子图) 生成报告
       └─ chat → chat_agent(子图) 对话答疑
```

- **5 个 Agent 子图**：prd_agent / sop_agent / chat_agent / executor_agent（单节点子图，主图游标串行调用）/ report_agent，各自独立 State schema、LLM 与提示词
...
│       ├── core/
│       │   ├── orchestrator.py    # 主图编排（router + 游标串行循环 + 统一操作 API）
│       │   ├── state.py           # MainGraphState（reducer 通道）
│       │   ├── llm.py             # 模型工厂（任务路由）
│       │   └── config.py          # 配置管理（.env 驱动）
│       ├── sop/models.py          # Pydantic 数据模型
│       ├── tools/                 # minium 工具层（会话管理 + 导航/截图/元素工具）
│       ├── prd/                   # （预留）PRD 解析
│       ├── executor/              # （预留）检查执行器
│       └── report/                # （预留）报告生成
│   └── tests/                     # pytest 单测（fake minium 注入 + 环境检测）
├── frontend/
│   ├── src/
│   │   ├── App.jsx                # 根组件
│   │   ├── components/
...
├── pyproject.toml
├── PITFALLS.md                    # 开发坑点记录（DeepSeek/LangGraph/minium 等）
├── .env                           # 环境变量（不提交）
└── .env.example
```
...
| 阶段四：检查执行 | 🟡 进行中 | ✅ 多 Agent 架构重构（5 个 Agent 子图 + 游标串行执行 + SSE 进度）；✅ minium 工具层接入（导航/截图真机验证，executor 双模式桩降级）；⬜ 元素查询兼容（Taro 运行时超时，备选 Qwen 视觉判定）；⬜ 聊天滚动摘要记忆 |
...
| 串行执行 | exec_cursor 游标 + 条件边自循环 | 微信开发者工具单实例约束，无法并行自动化（PITFALLS 6.4）；exec_results/agent_progress reducer 通道 + run_id 过滤隔离 |
| minium 导航 | app.evaluate 直调 wx API | minium hookNavigation 与自定义运行时冲突；绕开包装层（PITFALLS 6.3） |
| 工具异常 | 转文本回喂 LLM | agent 循环内不中断，异常转成 [工具执行失败] ToolMessage，LLM 可据此调整策略 |
| minium 开关 | MINIUM_ENABLED=true opt-in | 显式启用 + 双路径齐备才启用；环境缺失 executor 自动降级桩 |
...
- [x] 多 Agent 架构重构（5 个 Agent 子图 + SSE 进度；并行 Send 改游标串行）
- [x] minium 工具层接入（导航 evaluate 绕过 / 截图 / 元素工具；executor 双模式桩降级）
- [ ] 元素查询兼容：page.get_element/get_text 对 Taro 类运行时超时；备选 ① app.evaluate 直查 DOM ② xpath ③ Qwen 视觉判定（截图分析，下一步计划）
...
| 假设 | 风险 | 缓解措施 |
|------|------|----------|
| 微信开发者工具已安装 | 用户环境可能未配置 | is_available 检测（opt-in 开关 + 路径存在）；环境缺失 executor 自动降级桩 |
| minium 兼容目标小程序 | API 兼容性问题 | 导航/截图已真机验证；元素查询 Taro 运行时超时未解决（备选 Qwen 视觉判定） |
